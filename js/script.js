...
function isCovered(cp, colony) {
  const nc = normalizeStr(colony);
  const inDefault = defaultZones.some(z => z.cp === cp && (normalizeStr(z.name) === nc || z.name === 'Todo el CP'));
  if(inDefault) return true;
  return customZones.some(z => z.cp === cp && (normalizeStr(z.colony) === nc || z.colony === 'Todo el CP'));
}
